...
// --- AJUSTE FIREBASE ---
// Aviso para o TypeScript sobre a variável global do Firebase que adicionamos no index.html
...
// --- AJUSTE FIREBASE ---
// Bloco de configuração que lê as chaves secretas salvas na Vercel
...
// --- AJUSTE FIREBASE ---
// Flag para evitar que uma atualização recebida do Firebase dispare um novo salvamento, criando um loop infinito.
...
    if (!date) return 'N/A';
    // --- AJUSTE FIREBASE --- A data vinda do Firebase pode ser um objeto com segundos, precisamos converter para Date
    const dateObj = (date.seconds) ? new Date(date.seconds * 1000) : date;
    return dateObj.toLocaleDateString(locale === 'pt' ? 'pt-BR' : locale, {
        day: '2-digit',
        month: '2-digit',
        year: 'numeric',
...
    }
    ...
    // --- AJUSTE FIREBASE --- Handle Firestore timestamp object
...
    reader.onload = async (e: ProgressEvent<FileReader>) => { // --- AJUSTE FIREBASE: Adicionado async
        try {
...
            const processedData = processData(mappedData);
            
...
            }
            ...
            // --- AJUSTE FIREBASE ---
            // Em vez de atualizar o estado local diretamente, salvamos no Firebase.
            // O listener 'escutarMudancasEmTempoReal' vai receber os dados e atualizar a tela de todos.
            await saveDataToFirebase({
                allData: processedData,
                demurrageRates: appState.demurrageRates, // Mantém as taxas atuais
                paidStatuses: {}, // Reseta os status de pagamento
                lastUpdate: `<span data-translate-key="upload_prompt_updated">${translate('upload_prompt_updated')}</span> ${file.name} em ${new Date().toLocaleString()}`
            });
            
...
    if (appState.allData.length === 0) { // --- MODIFICADO FIREBASE: Usa allData pois filteredData pode estar vazio
...
    populateFilters();
    // --- AJUSTE FIREBASE --- Aplicar filtros antes de renderizar KPIs e colunas
    applyFilters(); 
    updateKPIs();
    renderColumns();
...
function updateLastUpdate(htmlContent: string) {
    lastUpdateEl.innerHTML = htmlContent;
}
...
    globalSearch((document.getElementById('global-search-input') as HTMLInputElement).value);
    // --- MODIFICADO FIREBASE: Não chama renderDashboard() aqui, pois já é chamado pelo listener ou pelo evento que iniciou o filtro.
}
...
async function saveRates() { // --- AJUSTE FIREBASE: Adicionado async
...
    // Recalculate cost
    const recalculatedData = appState.allData.map(container => {
        const shipowner = container.Shipowner.toUpperCase();
        const rate = appState.demurrageRates[shipowner] || appState.demurrageRates.default;
        return {
            ...container,
            'Demurrage Cost': container['Demurrage Days'] * rate,
        };
    });

    // --- AJUSTE FIREBASE ---
    // Salva o estado completo (novos dados + novas taxas) no Firebase
    await saveDataToFirebase({
        allData: recalculatedData,
        demurrageRates: newRates,
        paidStatuses: appState.paidStatuses, // Mantém os status de pagamento
        lastUpdate: lastUpdateEl.innerHTML // Mantém a última atualização
    });
...
// --- HISTORY MANAGEMENT ---
// O histórico continua a ser uma funcionalidade local do navegador (localStorage)
// ... (Nenhuma alteração necessária nas funções de Histórico)
...
    hideHistoryBanner();
    // --- AJUSTE FIREBASE: Não precisa recarregar do localStorage, os dados ao vivo já estão no estado. Apenas renderizamos.
    renderDashboard(); 
...
        checkbox.addEventListener('change', async (e) => { // --- AJUSTE FIREBASE: Adicionado async
            const target = e.target as HTMLInputElement;
            const containerId = target.dataset.containerId!;
            appState.paidStatuses[containerId] = target.checked;
            // --- AJUSTE FIREBASE --- Salva o estado completo no Firebase após a mudança
            await saveDataToFirebase({
...
// --- ANALYTICS/CHARTS ---
// Nenhuma mudança necessária aqui, pois os gráficos leem do `appState` que é atualizado pelo listener do Firebase.
// ... (código dos gráficos permanece o mesmo) ...

// --- PERSISTENCE ---

// --- AJUSTE FIREBASE: Nova função central para salvar no Firebase ---
async function saveDataToFirebase(dataToSave) {
    ...
    try {
        // Para evitar problemas com objetos Date, convertemos para strings ISO antes de salvar
        const serializableData = JSON.parse(JSON.stringify(dataToSave.allData));

        await db.collection("demurrageData").doc("current").set({
            ...dataToSave,
            allData: serializableData, // Salva os dados serializados
            timestamp: firebase.firestore.FieldValue.serverTimestamp() // Adiciona um carimbo de data/hora do servidor
        });
        console.log("Data successfully saved to Firebase!");
    } catch (error) {
        console.error("Error saving data to Firebase: ", error);
        showToast("Falha ao sincronizar dados. Verifique a conexão.", "error");
    }
}

// --- AJUSTE FIREBASE: Nova função para ouvir as mudanças em tempo real ---
function listenToFirebase() {
    console.log("Setting up Firebase listener...");
    db.collection("demurrageData").doc("current").onSnapshot(doc => {
        console.log("Received update from Firebase.");
        isUpdatingFromFirebase = true; // Ativa a flag para evitar loop

        if (doc.exists) {
            const data = doc.data();
            
            // Recarrega o estado do app com os dados do Firebase
            appState.allData = data.allData.map(d => {
                // É crucial re-converter as strings de data de volta para objetos Date
                return {
                    ...d,
                    'Discharge Date': parseDate(d['Discharge Date']),
                    'End of Free Time': parseDate(d['End of Free Time']),
                    'Return Date': parseDate(d['Return Date']) || undefined,
                };
            });
            ...
            updateLastUpdate(data.lastUpdate || 'Dados carregados do servidor.');
            renderDashboard();

        } else {
            // Se não houver dados no Firebase, reseta a UI
            console.log("No data in Firebase, resetting UI.");
            appState.allData = [];
            appState.filteredData = [];
            renderDashboard();
            updateLastUpdate(translate('upload_prompt_initial'));
        }

        // Desativa a flag após a renderização para permitir novos salvamentos
        setTimeout(() => {
...
// --- AJUSTE FIREBASE: A função de limpar dados agora limpa o Firebase ---
async function clearData() {
    if (confirm('Tem certeza de que deseja limpar todos os dados do servidor para todos os usuários? Esta ação não pode ser desfeita.')) {
        loadingOverlay.classList.remove('hidden');
        try {
            // Apaga o documento no Firebase
            await db.collection("demurrageData").doc("current").delete();
            // Limpa o histórico local
            localStorage.removeItem('demurrageHistory');
            showToast(translate('toast_clear_data'), 'info');
            // O listener do Firebase detectará a exclusão e limpará a UI automaticamente.
        } catch (error) {
...
// --- THEME & TRANSLATION ---
// ... (Nenhuma alteração necessária aqui, pois são configurações locais) ...
...
    applyFiltersBtn.addEventListener('click', () => {
        applyFilters();
        renderDashboard(); // Força a renderização após aplicar filtros manualmente
    });
...
    setupModals();
    setupFilterSearch();
    // --- AJUSTE FIREBASE: Inicia o listener que vai carregar os dados e manter tudo sincronizado
    listenToFirebase();
    // Carrega preferências locais que não são sincronizadas
    appState.currentLanguage = localStorage.getItem('demurrageLanguage') || 'pt';
    translateApp();
}
